...
GestureProcess::GestureProcess():loadFlag_(false), modelId_(0), modelMemPtr_(nullptr), modelMemSize_(0),
modelWeightPtr_(nullptr),modelWeightSize_(0), modelDesc_(nullptr), input_(nullptr), output_(nullptr),
isReleased_(false){

}

GestureProcess::~GestureProcess() {
    DestroyResource();
}

void GestureProcess::DestroyResource() {
    if (isReleased_)
        return;
...
Result GestureProcess::LoadModelFromFileWithMem(const char *modelPath) {
    if (loadFlag_) {
...
Result GestureProcess::CreateDesc()
{
...
void GestureProcess::DestroyDesc()
{
    if (modelDesc_ != nullptr) {
...
Result GestureProcess::CreateInput(void *input1, size_t input1size) {
...
void GestureProcess::DestroyInput() {
    if (input_ == nullptr) {
        return;
    }
...
Result GestureProcess::CreateOutput() {
    if (modelDesc_ == nullptr) {
...
void GestureProcess::DestroyOutput() {
    if (output_ == nullptr) {
        return;
    }
...
aclmdlDataset *GestureProcess::GetModelOutputData() {
    return output_;
}
